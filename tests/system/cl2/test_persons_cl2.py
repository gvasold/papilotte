...
def test_update_person(mockclient_cl2, person1):
    ...
    newdata["uris"] = ["https://example.com/persons1a", "https://example.com/persons2a"]
...
def _test_update_person_with_invalid_id(mockclient_cl2, person1):
    "Only specific chars are allowed as id."
    r = mockclient_cl2.put(TEST_URL + "/1f$oo(2", json=person1)
...
def test_create_person(mockclient_cl2, person1):
    "Test creatio of a person."
    person1["@id"] = "1001"
    r = mockclient_cl2.post(TEST_URL, json=person1)
    assert r.status_code == 200
    assert r.json["@id"] == "1001"

    # Creating the same person is not allowed
    r = mockclient_cl2.post(TEST_URL, json=person1)
    assert r.status_code == 409
    assert "already exists" in r.json["detail"]

# TODO: disabled because this is not part of the spec yet
def _test_create_person_with_invalid_id(mockclient_cl2):
    "Only specific chars are allowed as id."
    r = mockclient_cl2.put(TEST_URL + "/1foo(2", json={"@id": "[$a"})
    assert r.status_code == 400


def test_delete_person(mockclient_cl2):  # , person1):
    "Delete a person."
    # deleting a person requires that it is not used in any factoid,
    ...
    r = mockclient_cl2.delete(TEST_URL + "/Pxxxxxxx2")
    assert r.status_code == 404


def test_referential_integrity(mockclient_cl2, person1):
    """Deleting a Person, which is is referenced by factoids must lead to a 409.
    """
    r = mockclient_cl2.delete(TEST_URL + "/P00002")
    assert r.status_code == 409
