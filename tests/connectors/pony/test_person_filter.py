"""Tests for filtering (searching connectors.pony.person)
"""
import copy
import datetime
...
# ------------------ compliance level 0 get tests ------------------------------------

ALLOWED_FILTERS_CL0 = ['factoidid', 'from', 'memberof', 'name', 'place', 'relatestoperson', 
...
def test_filter_by_source_id(db200final_cfg):
    ...
    # searching for ids only matches full ids (not parts of id)
    assert connector.search(size=100, page=1, sourceId="F001") == []

# TODO: searching for labels is not in the spec yet. But we are prepared ;)
...
def test_filter_by_p(db200final_cfg):
    ...
    assert len(connector.search(size=100, page=1, p="P00058")) == 1
    assert len(connector.search(size=100, page=1, p="P0005")) == 10
    assert len(connector.search(size=100, page=1, p="https://example.com/persons/34a")) == 1
    # Test if only full uris are matched, so only '34', but not 34a
    assert len(connector.search(size=100, page=1, p="https://example.com/persons/34")) == 0


def test_filter_by_f(db200final_cfg):
    "Test the f_filter"
    connector = person.PersonConnector(db200final_cfg)
    assert len(connector.search(size=100, page=1, f="F00033")) == 1
    assert len(connector.search(size=100, page=1, f="F0003")) == 10


def test_filter_by_s(db200final_cfg):
    connector = person.PersonConnector(db200final_cfg)
    assert len(connector.search(size=100, page=1, s="S00033")) == 2
    assert len(connector.search(size=100, page=1, s="S0003")) == 20
    assert len(connector.search(size=100, page=1, s="Source 00031")) == 2
    assert len(connector.search(size=100, page=1, s="https://example.com/sources/56a")) == 2


# TODO: has to be extend when method is extended
...
# TODO: write some test for filtered counting    